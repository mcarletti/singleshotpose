...
def build_targets(pred_corners, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in range(nB):
        ...
        conf_mask[b][(cur_confs>sil_thresh).view(conf_mask[b].shape)] = 0
    if seen < -1:#6400:
       tx0.fill_(0.5)
       ty0.fill_(0.5)
       tx1.fill_(0.5)
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        pred_corners[0]  = (x0.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[1]  = (y0.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[2]  = (x1.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[3]  = (y1.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[4]  = (x2.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[5]  = (y2.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[6]  = (x3.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[7]  = (y3.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[8]  = (x4.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[9]  = (y4.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[10]  = (x5.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[11]  = (y5.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[12] = (x6.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[13] = (y6.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[14] = (x7.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[15] = (y7.data.view(nB*nA*nH*nW) + grid_y) / nH
        pred_corners[16] = (x8.data.view(nB*nA*nH*nW) + grid_x) / nW
        pred_corners[17] = (y8.data.view(nB*nA*nH*nW) + grid_y) / nH
